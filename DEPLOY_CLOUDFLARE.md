...
После этого пользователь на сайте нажимает `Telegram` и открывает выданную ссылку. Бот запрашивает собственный контакт, присылает код в Telegram, а пользователь вводит этот код на сайте. В боте доступны `/rating` и `/add Название | Жанр | Год | Рейтинг | пройдена`.
