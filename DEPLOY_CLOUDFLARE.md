...
Общий каталог хранится в таблицах `catalog_games`, `user_games`, `reviews`, `community_posts` и `sync_runs` в той же D1-базе, что сайт и бот. Пользователь публикует игру из личной библиотеки: Worker сначала проверяет её через RAWG, затем сохраняет проверенную игру в `catalog_games` и публикацию в `community_posts`. Worker запускает ежедневную синхронизацию RAWG в 03:00 UTC и обновляет уже опубликованные проверенные игры.
...
После этого пользователь может войти или зарегистрироваться через кнопку Telegram на экране авторизации. Бот запрашивает собственный контакт, присылает код в Telegram, а пользователь вводит этот код на сайте. Созданный таким способом пользователь сохраняется в той же базе, что и обычный аккаунт. В боте доступны `/rating` и `/add Название | Жанр | Год | Рейтинг [| пройдена]`; поля можно разделить символами `|`, `\` или `;`.
