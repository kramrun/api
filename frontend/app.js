...
$('#authSwitch').onclick = () => { authMode = authMode === 'login' ? 'register' : 'login'; const registering = authMode === 'register'; $('#authTitle').textContent = registering ? 'Регистрация' : 'Вход'; $('#authSubmit').firstChild.textContent = registering ? 'Создать аккаунт ' : 'Войти '; $('#authSwitch').textContent = registering ? 'Уже есть аккаунт? Войти' : 'Нет аккаунта? Зарегистрироваться'; $('#authPassword').autocomplete = registering ? 'new-password' : 'current-password'; $('#authPhoneField').hidden = !registering; $('#authPhone').required = registering; $('#authError').textContent = ''; };
async function createTelegramLink(phone) { const result = await request('/auth/telegram-link', {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({phone})}); $('#telegramStatus').innerHTML = `Откройте <a href="${esc(result.telegram_url)}" target="_blank" rel="noopener">бота в Telegram</a> и отправьте свой контакт. Ссылка действует 15 минут.`; window.open(result.telegram_url, '_blank', 'noopener'); }
$('#authForm').addEventListener('submit', async event => { event.preventDefault(); $('#authError').textContent = ''; const phone = $('#authPhone').value.trim(); const registering = authMode === 'register'; try { const result = await request(`/auth/${authMode}`, {method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({username: $('#authUsername').value.trim(), password: $('#authPassword').value})}); token = result.access_token; localStorage.setItem('checkpoint_token', token); showUser(result.user); await loadGames(); if (registering) { $('#telegramPhone').value = phone; $('#telegramDialog').showModal(); try { await createTelegramLink(phone); } catch (error) { $('#telegramStatus').textContent = error.message; } } } catch (error) { $('#authError').textContent = error.message; } });
$('#telegramButton').onclick = () => { $('#telegramStatus').textContent = currentUser?.telegram_linked ? `Telegram подключён${currentUser.phone ? `: ${currentUser.phone}` : ''}. Чтобы сменить номер, подтвердите новый.` : 'После подтверждения здесь появится статус подключения.'; $('#telegramPhone').value = currentUser?.phone || ''; $('#telegramDialog').showModal(); $('#telegramPhone').focus(); };
$('#closeTelegramDialog').onclick = $('#cancelTelegramDialog').onclick = () => $('#telegramDialog').close();
$('#telegramForm').addEventListener('submit', async event => { event.preventDefault(); const button = $('#telegramSubmit'); if (button.disabled) return; button.disabled = true; $('#telegramStatus').textContent = ''; try { await createTelegramLink($('#telegramPhone').value.trim()); } catch (error) { $('#telegramStatus').textContent = error.message; } finally { button.disabled = false; } });
