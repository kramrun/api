...
function render() { const term = $('#searchInput').value.trim().toLowerCase(); const shown = games.filter(g => { const matchesSearch = !term || `${g.title} ${g.genre}`.toLowerCase().includes(term); const matchesFilter = activeFilter === 'all' || (activeFilter === 'completed' ? g.completed : !g.completed); return matchesSearch && matchesFilter; }); grid.innerHTML = shown.map((game, i) => `<article class="game-card reveal" style="--accent:${genreColor(game.genre)}; --delay:${i * 55}ms"><div class="card-top"><span class="game-year">${game.year}</span><div class="card-menu"><button data-action="edit" data-id="${game.id}" aria-label="Редактировать">···</button></div></div><div class="game-art"><span>${esc(game.genre.slice(0, 1).toUpperCase())}</span><i></i></div><div class="game-info"><div><p>${esc(game.genre)}</p><h3>${esc(game.title)}</h3></div><b>${game.rating.toFixed(1)}</b></div><div class="card-bottom"><button class="status ${game.completed ? 'done' : ''}" data-action="complete" data-id="${game.id}"><span>${game.completed ? '✓' : '○'}</span>${game.completed ? 'Пройдено' : 'В процессе'}</button><button class="status" data-action="publish" data-id="${game.id}">Опубликовать</button><button class="delete" data-action="delete" data-id="${game.id}" aria-label="Удалить игру">×</button></div></article>`).join(''); $('#emptyState').hidden = games.length !== 0; updateStats(); }
...
async function openPublish(game) {
  ...
  try {
    ...
    $('#publishStatus').textContent = 'Выберите найденную игру и добавьте отзыв при желании.';
